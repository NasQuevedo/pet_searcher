...
const getLosts = () => {
    ...
    jQuery.ajax({
        url: './controller/pet/get_losts.php',
        method: 'POST',
        dataType: 'json',
        type: 'json',
        dataType: 'json',
        ...
        success: (response) => {
            switch (response.response) {
                case 'success':
                    ...
                    if (results.length > 0) {
                        jQuery("#lost-img").hide();
                        jQuery("#lost-table").show();
                        html = "<thead>" +
                                "<tr>" +
                                    "<th>Nombre</th>" +
                                    "<th>Sexo</th>" +
...
jQuery("#save").on('click', () => {
    ...
    const description = jQuery("#descripcion").val();
    let validated = true;
    ...
    formData.append('action', 'create_lost');

    if (validated) {
        jQuery.ajax({
            url: './controller/pet/create_lost.php',
            method: 'POST',
            dataType: 'json',
            data: formData,
            contentType: false,
                ...
                switch (response.response) {
                    case 'success': 
                        ...
                        /*if (response.response.warning !== '') {
                            jQuery("#warning-lost").html("Registro Existos! ".response.reponse.warning);
                            jQuery("#warning-lost").show();
                        } else {*/
                            jQuery("#success-lost").html('Registro Exitoso! esperamos encuentres a tu mascota');
                            jQuery("#success-lost").show();
                        //}
                        getLosts();
                        break;
                    case 'error_insert':
                        jQuery("#error-lost").html('Error al Registrar!');
                        jQuery("#error-lost").show();
                        break;
                    case 'error_request':
                        jQuery("#error-lost").html('Algo Salio Mal!');
                        jQuery("#error-lost").show();
                        break;
                }
            }
